...
const officeLocations: OfficeLocation[] = [
  {
    name: "Silver Dollar Wood Products LLC",
    address: "100 N, NM-445, Maxwell, NM 87728, United States",
    phone: "+1 (575) 375-1234",
    coordinates: [36.5403, -104.5436] // Maxwell, New Mexico
  }
];
...
export default function MapSection() {
  ...
  const recreateAllMarkers = (map: L.Map, shouldFitBounds: boolean = true) => {
    ...
    // For single location, don't auto-fit bounds to keep zoomed out view
    if (shouldFitBounds && bounds.isValid() && officeLocations.length > 1) {
      map.fitBounds(bounds, { padding: [20, 20] });

      // Ensure we never zoom out below the desired default
  ...
  const getDefaultZoom = () => {
    ...
    return window.innerWidth < 768 ? 5 : 6; // Mobile uses 5, others 6 - more zoomed out
  };
  ...
  const initializeMapWithState = (expandedState: boolean) => {
    ...
    const map = L.map(mapRef.current, {
      center: [36.5403, -104.5436], // Center on Maxwell, NM office
      zoom: defaultZoom,
      zoomControl: true,
    ...
    // Enforce minimum zoom (esp. on mobile) so users can't zoom out too far
    map.setMinZoom(Math.max(4, defaultZoom - 2)); // Allow zooming out 2 levels below default
